...
>> 버전 정하기
   현재 버전: v1.2.1
     ...
     1) 1.2.2      패치 - 버그 수정만
     2) 1.3.0      마이너 - 기능 추가
     3) 2.0.0      메이저 - 설정 형식이 바뀌어 되돌릴 수 없을 때
     4) 직접 입력

   어느 버전으로 올릴까요? [1]: 2

>> v1.3.0 업데이트 내용
   이 버전에서 바뀐 내용을 한 줄에 하나씩 적어주세요.
   업데이트 알림 창에서 사용자가 그대로 보게 됩니다.
   다 적었으면 빈 줄에서 Enter를 누르세요.

   - 가격 확인 창 밖을 클릭하면 닫힙니다
   - 지도 옵션 정규식 탭 추가
   -
```

여기서 적은 내용이 **그대로** GitHub 릴리즈 노트와 사용자의 업데이트 알림 창에 올라갑니다.
빈 줄에서 바로 Enter를 누르면 예전처럼 커밋 메시지로 자동 작성합니다.

커밋하지 않은 변경이 있으면 목록을 보여주고 커밋할지 물어봅니다 —
따로 `git commit` 하고 다시 시작할 필요는 없습니다.

터미널에서 쓸 때:

```powershell
release.bat                # 위와 동일
release.bat --dry-run      # 빌드까지만, 푸시/업로드 안 함
release.bat 1.3.0          # 버전 질문 건너뛰기
```

직접 `release.py`를 부르는 예전 방식도 그대로 됩니다:
...
> **빌드 전에 실행 중인 프로그램을 종료하세요.**
> `dist\KuanPoeHelper.exe`가 실행 중이면 파일을 교체할 수 없어 빌드가 멈춥니다.
> 창의 X 버튼이나 트레이 아이콘 → 종료로 완전히 끝납니다.
...
- [ ] 관리자 권한(UAC) 요청이 뜬다
- [ ] 탭이 전부 열린다 (특히 게임 단축키, 좌표 설정)
- [ ] 단축키가 게임에서 동작한다
- [ ] 설정 탭 → 자동 업데이트 → **지금 확인** → "최신 버전입니다"
